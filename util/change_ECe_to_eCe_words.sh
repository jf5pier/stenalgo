...
awk -F'\t' -v OFS='\t' '$2 ~ /E[RtsplkmdvjnfbZwzSgNG]{1,4}e$/ { print $3 }' resources/Lexique383.tsv | sort -u >lemmes_possible_ECe_corretion

awk -F'\t' 'NR==FNR { lemmes[$0]; next } $3 in lemmes && $2 ~ /e[RtsplkmdvjnfbZwzSgNG]{1,4}e$/ { print $1 }' lemmes_possible_ECe_corretion resources/Lexique383.tsv >words_ECe_correction

awk -F'\t' 'NR==FNR { lemmes[$0]; next } $3 in lemmes && $2 ~ /e[RtsplkmdvjnfbZwzSgNG]{1,4}e$/ {
...
awk -F'\t' 'NR==FNR { words[$0]; next } $1 in words  {
  print $1 "	" $2 "	" $5 }' words_ECe_correction resources/LexiqueInfraCorrespondance.tsv

awk -F'\t' 'NR==FNR { words[$0]; next } $1 in words && $2 ~ /e[RtsplkmdvjnfbZwzSgNG]{1,4}e$/ {
  PHON = gensub(/e([RtsplkmdvjnfbZwzSgNG]{1,4}e)$/, "E\\1", "1", $2);
