# Realization Phase (physical realization) — Milestone 1, DONE
...
historical context), `ATOMIC_KEYPRESS_REWIRE_PLAN.md`'s "Realization Phase" section, and
`RESUME_2026-09-19-cpsat.md`/`RESUME_2026-09-19-phaseG.md` (Grouping Phase's adopted output
this milestone consumes: `keypress_groups.json`).

## Bottom line

Realization Phase milestone 1 is complete and verified against the real 47,799-group,
~136k-word lexicon: **all 6 of Grouping Phase's abstract keypress groups now have a real
physical coda key assignment, with zero collisions left in Realization Phase's own
jurisdiction.** Output artifact: `realization_report.json`, built by
...
below): **0 theory collisions, 0 same-lemmeGramCat collisions** (Realization Phase's own job --
fully resolved), **29 cross-category clashes** (a separate, already-documented bug
class, not Realization Phase's), **1,252 cross-lemma collisions** (the `*`/`#` reserved-key
track's job, not yet implemented). These numbers can drift slightly between runs of
`util/build_realization_report.py` if the underlying lexicon/pickles change, but the
...
1. **The discriminator is a brand-new trailing stroke, not merged into the last
   ...
   the word's LAST existing stroke's chord. The real Realization Phase code
   ...
   syllable's own keys.
2. **A word needing several Grouping Phase groups at once gets ONE shared extra stroke,
...
1. **Same `lemmeGramCat`, different `ortho`** -- two inflected forms of the exact same
   lemma+gramCat paradigm (e.g. `dors`/`dort`). This is the ONE thing Grouping/Realization Phases'
...
2. **Same bare `lemme`, different `gramCat`** (e.g. `dîner` the NOM vs `dîner` the
   ...
   separate, pre-existing issue class. NOT Realization Phase's job; reported via
...
3. **Different `lemme` entirely** (true different-word homophones, e.g.
   ...
   `theory`. NOT Realization Phase's job; reported via `crossLemmaCollisions`.
...
bucket-2 pairs as if they were Realization Phase's problem and inflated the apparent failure
...
Mid-session, in response to a totally reasonable-sounding argument ("Grouping Phase already
...
Why the simple version is insufficient: Grouping Phase's own `verifyKeypressAssignment`
(`src/featuregrouping.py:142`) only proves the 6 groups are pairwise distinguishable in the
...
moment every Grouping Phase group it needs has been processed (assigned a key, or given up
...
- `buildKeypressGroupToWords(resolvedGroups, markersByKeypress, wordToStrokes,
  wordsByOrthoLemme)` -- maps each Grouping Phase group id to every real `Word` whose
...
`util/build_realization_report.py` -- the canonical build script (mirrors
`util/build_keypress_groups.py`'s role). Loads `Dictionary.pickle`/
`FirstTheory.pickle`, `keypress_groups.json`, `resolved_press_sets.json`;
runs `realizeKeypressGroupsAsExtraStroke`; writes `realization_report.json`
...
`src/test/ambiguitychecker_test.py` (all Realization Phase functions above, including the two
...
- Rewriting `dictionary.py`'s full word list with resolved strokes (replacing
  ...
  `realization_report.json` artifact, per ROADMAP.md open question 4.
...
mypy src/ambiguitychecker.py src/keyboard.py src/cpsatsolver.py src/greedyoptimizer.py util/build_realization_report.py
python -m util.build_realization_report
```
The last command should print `6/6 keypress groups realized (0 residual theory
collisions, 0 residual same-lemmeGramCat collisions [Realization Phase's own job], ...)`. If the
