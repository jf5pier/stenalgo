...
# (without it, realization_report.json residual lists vary run to run).
export PYTHONHASHSEED=0
rm -f Dictionary.pickle FirstTheory.pickle
$PY dictionary.py                          >&2
$PY -m src.elicitation                     >&2
$PY -m util.build_keypress_groups       >&2
$PY -m util.build_realization_report      >&2
...
git checkout -- realization_report.json
