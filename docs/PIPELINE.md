...
- Stages and phases are always cited by their descriptive name with the code in
  parentheses, for example "Marker Grouping (Phase G)". Calls are numbered in execution
  order inside their stage (for example "Theory 1 construction (S2.11)"); the number always
  follows a descriptive name.
- Terms in **bold** (at first use) and every dataset-state name are defined in
  [GLOSSARY.md](GLOSSARY.md), in this folder. Terms marked there as
  "provisional" may still be renamed.
...
|---|---|---|---|---|
| 0 | `python -m util.fix<Name> --apply`, `python -m util.completeVerbParadigms --apply`, `python -m util.generateMissingNomAdjForms --apply`, … | Lexicon Building (S1), Synthetic Paradigm Completion (S1b) | only after a lexicon correction | Patch `Lexique383.tsv`, `LexiqueInfraCorrespondance.tsv`, Verbiste XML and/or `LexiqueMixte.tsv`, or append rows to `LexiqueSynthetic.tsv`. Run by hand, one fix at a time. |
...
| 4 | `python -m src.elicitation` | Marker Elicitation (Phase E) | everything after it | Rebuilds the resolved press-sets from the stored `elicitation_answers.json`. Asks no questions. |
| 4h | `python -m util.build_questionnaire_page` → publish → answer → copy answers into `elicitation_answers.json` → rerun step 4 | Marker Elicitation (Phase E) | only when step 4 reports "unresolved oppositions" | The human-in-the-loop part. |
| 5 | `python -m util.build_phase_g_assignment` | Marker Grouping (Phase G) | when the live markers or press-sets change | The tracked output rarely changes after a lexicon fix. |
| 6 | `python -m util.build_phase_p_realization` | Marker Stroke Realization (Phase P), report build | before step 9's keyboard legend | Writes the realization report only. It does not feed theory 2 or the Plover dictionary. |
| 7 | `python dictionary.py` (second run) | Phonetic Theory Building (S2) → Lemma-Homophone Marking (S4) | only to refresh `theory2.tsv` | Fast (pickles exist). |
| 8 | `python -m util.export_plover_dictionary`, `python -m util.export_plover_system` | Theory Export (S5) | Plover | Either order. |
| 9 | `python -m util.export_keyboard_layout` (after step 6), `python -m util.export_practice_words`, **then** `python -m util.export_practice_sentences`, then `python -m util.export_definitions` | Theory Export (S5), trainer branch | steno-trainer | `export_practice_sentences` reads `practice-words.json` (export_practice_sentences.py:157). |
| opt | `python -m util.check_conjugation_disambiguation_order` | Marker Elicitation (Phase E), validator | checking answers | Writes `conjugation_disambiguation_report.json` (gitignored). |
...
1. **Nothing reads `theory2.tsv`.** It is a gitignored human view. Every exporter that
   needs theory 2 recomputes it in its own process: `util/_theoryio.loadFirstAndFinalTheory`
   unpickles the dictionary and theory 1, then calls `Dictionary.buildFinalTheory`
   (util/_theoryio.py:82), which reruns Marker Stroke Realization (Phase P) and
   Lemma-Homophone Marking (S4). A full export computes theory 2 four times. The second
   `dictionary.py` run (step 7) is therefore needed only for `theory2.tsv`; the Plover
   dictionary is correct without it as long as steps 4 and 5 have run.
...
3. **`PYTHONHASHSEED` changes pickle contents.** Each Word's identity hash (`Word._hash`,
   ...
   Word is built and stored in the pickles. Marker Stroke Realization (Phase P) iterates a
   `set` of Words when it lists residual collisions, so a clean rebuild (new pickles, new
   hash values) reorders those lists in `phase_p_keypress_realization.json`. With the same
   pickles, four different seeds gave identical `theory2.tsv`, realization report and Plover
   dictionary; fresh pickles changed only the report's residual lists. Pin
   `PYTHONHASHSEED` for the run that writes the pickles when the tracked report must be
   reproducible. See TODO.md § Suspected bugs, item B11.
4. **The realization report is read by one exporter.** `export_keyboard_layout.py:128`
   takes the conjugation-marker legend from the tracked `phase_p_keypress_realization.json`,
   while the Plover dictionary and drills use the keys recomputed inline. Skip step 6 after a
   change of keypress groups and the legend disagrees with the dictionary
   (item B18).
5. **`starboard3h.json` is never written by the pipeline.** Eight error messages say "run
   dictionary.py once first to generate it"; `dictionary.py` only reads it (the
   `toJSONFile` call is commented out at dictionary.py:496).
...
| **synthetic lexicon rows** | same TSV plus a `source` column; 42,225 rows | Synthetic Paradigm Completion (S1b) scripts | `resources/LexiqueSynthetic.tsv` (tracked) |
...
| **syllable statistics** | `SyllableCollection` + `Syllable.*ColByPart` class state | `analyseSyllabification` dictionary.py:169, `optimizeBiphonemeOrder`, `analyseAmbiguities` | `Dictionary.pickle` (objects 1-5) |
| **keyboard layout** | `Starboard` (26 keys; reserved keys 0, 1, 10, 15) | `Keyboard.fromJSONFile` keyboard.py:252 | `starboard3h.json` (tracked, never rewritten) |
...
| **physical keypress assignment** | `KeypressGroupPhysicalAssignment` (`chosenKeysByGroup`, cost, alternates, residual buckets) | `realizeKeypressGroupsAsExtraStroke` ambiguitychecker.py:987 | report build only: `phase_p_keypress_realization.json` (tracked) |
| **final induced strokes** | `dict[Word, Strokes]`: base strokes plus at most one marker stroke | `buildFinalInducedStrokes` ambiguitychecker.py:1261 | no |
| **theory 2** | `dict[Word, list[Strokes]]`: index 0 primary (marked), then self-homograph alternates | `Dictionary.buildFinalTheory` dictionary.py:342 | `theory2.tsv` (gitignored, read by nothing) |
...
Synthetic Paradigm Completion (S1b) .............. manual --apply scripts  → LexiqueSynthetic.tsv
├─ Verb paradigm completion — completeVerbParadigms ............... S1b.1
├─ NOM/ADJ gap generation — generateMissingNomAdjForms ............ S1b.2
└─ Dual-form gap fillers, in-place repair ......................... S1b.3, S1b.4

Phonetic Theory Building (S2) .................... python dictionary.py
├─ Lexicon reading and identity merge — readCorpus ................ S2.2.1  (Word list)
├─ Syllable inventory, phoneme order search, ambiguity statistics . S2.3-S2.5 (off the live path except S2.3)
├─ Keyboard layout loading — Starboard.fromJSONFile ............... S2.7
│  └─ Keyboard Layout Optimization (S2b) — optimizeKeyboard ....... S2.9  (not run)
└─ Theory 1 construction — buildTheory ............................ S2.11 → FirstTheory.pickle, theory.tsv

Same-Lemma Disambiguation (S3)
├─ Marker Elicitation (Phase E) .................. python -m src.elicitation
│  ├─ Homophone group building — buildLemmaHomophoneGroups ........ S3.E.1
│  ├─ Questionnaire item selection — buildQuestionnaireItems ...... S3.E.4 → questionnaire.json
│  ├─ Press-set resolution — resolveGroupPressSets ................ S3.E.6
│  └─ Resolved press-set serialization ............................ S3.E.9 → resolved_press_sets.json
├─ Marker Grouping (Phase G) ..................... python -m util.build_phase_g_assignment
│  ├─ Exact minimum-K grouping — minKeypressesSatWithPriorities ... S3.G.2
│  └─ Assignment serialization .................................... S3.G.5 → phase_g_keypress_assignment.json
└─ Marker Stroke Realization (Phase P) ........... inline in buildFinalTheory, and report build
   ├─ Keypress group population — buildKeypressGroupToWords ....... S3.P.2
   ├─ Coda key search — realizeKeypressGroupsAsExtraStroke ........ S3.P.5
   ├─ Final induced strokes (inline) — buildFinalInducedStrokes ... S3.P.6
   └─ Report serialization (report build) ......................... S3.P.8 → phase_p_keypress_realization.json

Lemma-Homophone Marking (S4) ..................... inside Dictionary.buildFinalTheory (S4.1)
├─ Lemma-homophone cluster detection — groupHomophonesByReservedStroke  S4.5
├─ Mark code assignment + marking rule stack ...................... S4.7-S4.11
├─ Mark merge into the last phoneme stroke ........................ S4.13
└─ Theory 2 report — writeFinalTheory ............................. S4.15 → theory2.tsv

Theory Export (S5) ............................... python -m util.export_*
├─ Theory 2 loading (recomputes theory 2) — loadFirstAndFinalTheory  S5.1
├─ Plover dictionary export ....................................... S5.3 → plover_stenalgo_dictionary.json
├─ Plover key table export + plugin ............................... S5.4, S5.5 → _generated_keys.py
└─ Trainer exports ................................................ S5.6-S5.9 → steno-trainer/public/data/*.json
```

The two homophone problems have two mechanisms. Words that are forms of the same lemma and
grammatical category (dors/dort) are separated by Same-Lemma Disambiguation (S3), using
grammatical markers the user chose, realized as one extra coda-bank stroke. Words with
different lemmas or categories that still sound alike (ver/vert/verre, appel/appelle) are
separated by Lemma-Homophone Marking (S4), using the `*` and `#` keys.
...
(an **excluded word** is dropped in Phonetic Theory Building (S2)) or
...
one LemmeGramCat, so Same-Lemma Disambiguation (S3) handles them, not Lemma-Homophone
Marking (S4). Otherwise `spellingVariantLemme` (`ile→île` + 55 reform lemmas), keyed by lemma only.
...
Notes: only the first occurrence is fixed; a second cluster in one word stays merged.
...
first-seen tie-breaks in Phonetic Theory Building (S2).
...
(src/featureextractor.py:31); then the five steps below, Load theory 1 (S1b.1.1) to Append
synthetic rows (S1b.1.5).
...
Notes: its gating depends on the retired discriminator design. Not idempotent (item B13).

- **Load theory 1 — loadTheoryAndKeyboard (S1b.1.1)** :91 — unpickles `FirstTheory.pickle`,
  or builds a `Dictionary` and theory 1 in memory without writing. A stale pickle hides rows
  appended since the last rebuild.
- **Derive conjugation ending tables — deriveConjugationEndingTables (S1b.1.2)**
...
- **Find structural candidates — findStructuralCandidates (S1b.1.3)** :165 — for each
  **undersampled lemma** found by Detect undersampled lemmas (S1b.1.3.1), fills missing
  participle gender/number slots with Generate missing participles (S1b.1.3.2) and missing
  ...
  (S1b.1.3.3), gated by `MIN_FINITE_MATCH_RATE = 1.0` (:73). Result: (lemmeGramCat, info,
  generated Word, reference Word) candidates.
  - *Detect undersampled lemmas (S1b.1.3.1)* verbparadigm.py:681 — per trusted template, a
  ...
  - *Generate missing participles (S1b.1.3.2)* :367 — ortho from the Verbiste `par:pas`
  ...
  - *Generate missing finite forms (S1b.1.3.3)* :577 — ortho = Verbiste radical + ending;
    each phonological field = infinitive value truncated by the suffix length + the table
    ending (:611); `None` below the match rate.
- **Confirm by legacy discriminator collision — confirmCandidates (S1b.1.4)** :266 —
...
- **Append synthetic rows — writeSynthetic (S1b.1.5)** :324 — appends one line per
...
Enumerate missing slots (S1b.2.2), then uses Exception-table override (S1b.2.3) or
Morphalou-authoritative form (S1b.2.4). Skips a candidate whose ortho already exists for the lemma in either TSV
...
- **Derive NOM/ADJ ending tables (S1b.2.1)** nomAdjParadigm.py:250 — lemmas with ≥2 attested
...
- **Enumerate missing slots (S1b.2.2)** :132 / :166 — NOM expects `s`/`p` per attested gender,
...
- **Exception-table override — overrideCandidate (S1b.2.3)** generateMissingNomAdjForms.py:77
  ...
  See item B7.
- **Morphalou-authoritative form (S1b.2.4)** nomAdjParadigm.py:522 → `generateMissingForm`
...
#### Dual-form gap fillers (S1b.3) and in-place synthetic repair (S1b.4)
...
Called by: Dictionary construction (S2.2), in Phonetic Theory Building (S2).
Transformation: reads `wordSources = [LexiqueMixte.tsv, LexiqueSynthetic.tsv]`
(dictionary.py:66-69) in order and ignores `source`. A synthetic row with an existing
identity only merges `infover`; its syllabification and frequency are discarded. This is the
only pipeline consumer; other readers are generators and validators
...
| fixAsseoirDualFormGaps | Synthetic (append) | no | n/a | ass:eoir alternants (S1b.3) |
...
| fixEvaserWordFinalZSyllabification | Synthetic (in place) | no | n/a | évaser coda z (S1b.4) |
...
| fixPayerDualFormGaps | Synthetic (append) | no | n/a | pa:yer i/y twins (S1b.3) |
...
| completeVerbParadigms | Synthetic (append) | no | n/a | Verb paradigm completion (S1b.1) |
| generateMissingNomAdjForms | Synthetic (append) | no | n/a | NOM/ADJ gap generation (S1b.2) |
...
lexicon rows (42,225) into the Word list (167,639 Words, sorted by descending film
frequency), registers every spoken syllable (5,866 distinct) in the syllable statistics,
loads the committed keyboard layout and maps every Word to its **base strokes**, one stroke
per syllable. The result is theory 1, `dict[Strokes, list[Word]]` with 80,725 entries.
Words with the same raw Strokes land in the same **theory-1 entry**; this is where homophones
first appear. Everything later starts from theory 1. Phoneme order search (S2.4) and
Ambiguity statistics (S2.5) still run on every fresh rebuild, but only the uncalled layout
solver uses their output.
...
1. **Syllables come from the lexicon.** `syll_cv` (e.g. `p_E_R|d_R_#`) is split on `|` into
   ...
   (`phonemesToSyllableNames(withSilent=False)`, src/word.py:341). Four **syllable-break
   exceptions** can move a break during Word construction (S2.2.1.1).
...
3. **Each phoneme becomes its layout key tuple** for its part (see Phonetic stroke
   rule (S2.11.1)), concatenated onset → nucleus → coda in spoken order, kept whole: no sort, no
...
Called by: Phonetic Theory Building (S2) entry (`python dictionary.py`, `__main__` :448).
...
`multiphonemeColByPart`, assigned back onto the class, :453-457) and skips Dictionary construction (S2.2) through Dictionary cache write (S2.6). No
staleness check.
...
Called by: Dictionary cache check (S2.1), on a miss.
...
Input state: mixed lexicon, then synthetic lexicon rows (`wordSources`, :66-69); a missing file is skipped silently (:117).
Transformation:
...
- **Identity merge** (:113-137): identity = `(ortho, phon, lemme, cgram, genre, nombre)`. A
  ...
  (`mergeInfoVerb`) or is dropped. 10,990 rows fold (10,973 with a tag, 17 without). **The first
  row's syllabification and frequency win**: 5,369 folded rows had another `syll_cv`, 6,028
  another film frequency (item B9).
- Otherwise builds a `Word` (Word construction (S2.2.1.1)) and indexes it in `wordsByLemme[lemme]` (bare lemma)
  and `wordsByOrtho[ortho]`.
...
- **e+n syllable repair — fix_e_n_en (S2.2.1.1.1)** word.py:202 — `@|n_` → `@|`, `e|n_` →
  `en|` ("enivre"), recursively; every row.
- **-rdre infinitive split (S2.2.1.1.2)** word.py:215 — verbs only; a last syllable ending
  `_R_d_R_#` splits off `d_R_#` (ortho `_r_d_r_e` alike): `perdre` = `pER|dR`, so it does not
  canonicalize onto `perdent`/`perde`.
- **-ayer conditionnel split (S2.2.1.1.3)** word.py:248 — verbs only; `_E_#` + `R_j_` →
  `…|R|j_…` (ortho `r_i_(ez|ons)` alike): `paieriez` = `pE|R|je`; onset `R`=8 ⊂ `j`=(8,9) would
  otherwise merge it with `payez`.
- **-uer glide split (S2.2.1.1.4)** word.py:297 — verbs only; a last syllable ending `_8_a` or
...
Result: one Word with the union of **readings** (e.g. synthetic `sub:pre:1s` added to a
Lexique383 `ind:pre:1s` row). The incoming `syll_cv` is not compared.

### Syllable inventory — Dictionary.analyseSyllabification (S2.3)   dictionary.py:169
Called by: Phonetic Theory Building (S2) entry, after Dictionary construction (S2.2).
...
(Syllable registration (S2.3.1)) with weight `frequency`, or 0 for the 200 frequent words (:174).
Result: syllable statistics: 5,866 `Syllable` objects + class-level phoneme, biphoneme and multiphoneme collections.
Notes: `zip` truncates for the 99 Words whose two syllable lists differ in length (item B28).

- **Syllable registration — updateSyllable (S2.3.1)** grammar.py:803 — creates
  `Syllable(name, spelling)` on first sight, adds frequency (Frequency accumulation (S2.3.1.2)), records the Word in
  `phonoWords[phonology]` (`trackWord` :612). `buildTheory` later looks syllables up here.
- **Onset/nucleus/coda decomposition — Syllable.__init__ (S2.3.1.1)** grammar.py:468 —
  resolves each character to a shared `Phoneme` (`ValueError` for anything outside the 16
  nucleus, 20 consonant and 1 temporary `x` phonemes); classifies by position (rule 2 above;
  0 syllables have a consonant between two vowels; 237 have ≥2 vowels, almost all with `8`,
  e.g. `l8i`); counts per-part phonemes, ordered consonant pairs and cross-vowel pairs as
  `Biphoneme`s (:525-561) and the whole part as a `Multiphoneme` (:562-567): 184 onset, 28
  nucleus, 141 coda multiphonemes. `phonemeNamesByPart()` (:569) feeds the stroke rule.
- **Frequency accumulation — increaseSpellingFrequency (S2.3.1.2)** grammar.py:586 — adds
...
- **Collection sorting — sortPhonemesCollections (S2.3.2)** grammar.py:592 — sorts
  phonemes and biphonemes by frequency (read only by the fallback keymap and the solver).
...
difference of p1-before-p2 vs after.
Result: **best permutation** and **pairwise order matrix** per part. Today: onset
`dZksvptgzSmnbflNRwj`, coda `bjgpfwsktdvRNzlmnSZ`, nucleus `8ieE§5Oao92@`. README.md:92-146 shows these.
Notes: **no live consumer.** Read only by Fallback keymap (S2.8), Keyboard Layout
Optimization (S2b) (not run), `writeConstrainFiles` (call commented :481) and `printBarchart`.
...
Result: `syllabicAmbiguity`/`lexicalAmbiguity` 190/120/190 pairs (onset/nucleus/coda);
`syllabicPartAmbiguity` 16,836/378/9,870 pairs, zeros included.
Notes: **no live consumer** (fallback keymap, uncalled solver, commented calls only). The slowest step of a fresh rebuild.

### Dictionary cache write — pickle.dump ×5 (S2.6)   dictionary.py:467-472
Called by: Phonetic Theory Building (S2) entry, on a cache miss.
...
Called by: Phonetic Theory Building (S2) entry (dictionary.py:488).
...
`FileNotFoundError` becomes `None`.
...
Notes: first committed in 37fdc4e (2026-09-13), never rewritten; produced by an earlier,
uncommitted run of Keyboard Layout Optimization (S2b). "3h" is apparently solve time
...
Called by: Phonetic Theory Building (S2) entry, **only** if `starboard3h.json` is missing (:489-492).
Transformation: per part, the N most frequent phonemes (N = 8/4/10 single keys) get single keys in
best-permutation order; the rest take 2-, 3-, 4-key strokes (`getPossibleStrokes`
...
Result: an in-memory layout, never saved; every exporter would still fail without `starboard3h.json`.
...
Called by: nothing (dictionary.py:494 commented out). The import at :32 still loads OR-Tools
on every `import dictionary`, exporters included.
Design rationale, kept because it explains the committed layout. One independent CP-SAT
model per syllabic part (separate key banks):
- *Decision:* each phoneme gets exactly one stroke among all legal 1-4-key strokes of its
  bank (`getPossibleStrokes`, ordered by `strokeIsLowerThen`), using 1 to
  `maxKeysPerPhoneme` (5/4/5) keys. Sharing a stroke is allowed but penalized. The current
  layout is given as hints.
- *Ambiguity term:* a phoneme group's key set is the union of its phonemes' keys (which
  models digraph subsumption). For the 2,000 most ambiguous group pairs
  (`MAX_MULTIPHONEMES`), identical key sets cost `syllabic-part ambiguity ×
  AMBIGUITY_PENALTY` (30,000).
- *Ergonomic term:* `phoneme frequency × stroke cost × STROKE_ASSIGNMENT_PENALTY` (1).
  Stroke cost = sum of `FingerWeights` per finger press (keyboard.py:47-71) + zig-zag and
  row-gap penalties, × 0.85^fingers (`getStrokeCost` keyboard.py:547). An illegal finger
  combination has no cost and cannot be chosen.
- *Order term:* `pairwise order score × ORDER_PENALTY` (500), signed by which stroke is
  further left; a shared stroke pays half. Rewards layouts that read left-to-right in spoken order.
- *Objective:* minimize the sum; `SOLVER_TIME` = 90 s per part.
Result (if run): `keyboard.clearLayout()` then the solved strokes; destructive, not
persisted (item B32).
Notes: with frequencies per million, one ambiguous pair outweighs almost any ergonomic or
order gain — consistent with the committed layout sharing entries only among rare phonemes
(`N`, `G`, `9`, `O`, `w`). See also `src/cpsatprinter.py`.

### Theory-1 cache check — __main__ (S2.10)   dictionary.py:498
Called by: Phonetic Theory Building (S2) entry.
Transformation: loads `FirstTheory.pickle` if present and skips Theory 1 construction (S2.11) through
Theory-1 cache write (S2.13). Not keyed on
`Dictionary.pickle` or `starboard3h.json` (item B15).
Result: theory 1.

### Theory 1 construction — Dictionary.buildTheory (S2.11)   dictionary.py:305
Called by: Theory-1 cache check (S2.10) on a miss; also `util/completeVerbParadigms.py:109`.
...
(`KeyError` if missing), turns it into a stroke (Phonetic stroke rule (S2.11.1)) and appends the Word to
`theory[raw Strokes]`.
...
Called by: Theory-1 cache check (S2.10), on a miss.
...
under Lemma-Homophone Marking (S4).
...
S3.E.2 Reading enumeration — wordFeatureCombinations / featureCombinationsByOrtho (:30, :86)
S3.E.3 Scale report — reportScale (:164)                                        [print only]
S3.E.4 Questionnaire item selection — buildQuestionnaireItems (:220)
S3.E.5 Answer indexing — buildAnswersByOpposition (:279)
S3.E.6 Press-set resolution — resolveGroupPressSets (:374)
  S3.E.6.1 Per-reading press resolution — resolvePressByCombination (:312)
S3.E.7 Press-set conflict validation — validateElicitation (:425)
S3.E.8 Per-spelling frequency table — buildFrequencyByGroupOrtho (:449)
S3.E.9 Resolved press-set serialization — serializeResolvedPressSets (:471)
...
S3.G.2 Exact minimum-K grouping — minKeypressesSatWithPriorities (src/phasegsat.py:490)
  S3.G.2.1 Group shape dedup — groupSignatures (:40)
  S3.G.2.2 Minimum-K scan — minKeypressesSat → _feasibleAssignment (:417, :183)
  S3.G.2.3 Distinctness model — _buildDistinctnessModel (:51)
  S3.G.2.4 Hard constraints — _aloneAndMustDifferPairs / _addMustDifferPairs (:120, :108)
  S3.G.2.5 Soft preference tiers — _bestAssignmentWithPriorities (:358)
  S3.G.2.6 Alphabetical tie-break — _breakTiesAlphabetically (:153)
S3.G.3 Ground-truth verification — verifyKeypressAssignment (src/phaseg.py:160)
S3.G.4 Usage weights — frequencyWeightedChordSizes (src/phaseg.py:189)          [report only]
S3.G.5 Assignment serialization — serializeAssignment (src/phasegsat.py:530)

Marker Stroke Realization (Phase P) — inline in Dictionary.buildFinalTheory (dictionary.py:373-389)
...
S3.P.2 Keypress group population — buildKeypressGroupToWords (:803)
  S3.P.2.1 Entry-to-Word resolution — _resolveEntryWord (:776)
S3.P.3 Extra alternate population — buildKeypressGroupExtraAlternates (:844)
S3.P.4 Preferred key resolution — resolvePreferredKeysByGroup (:952)
S3.P.5 Coda key search — realizeKeypressGroupsAsExtraStroke (:987)
  S3.P.5.1 Candidate ranking — _bestCandidate (:1177)
  S3.P.5.2 Candidate feasibility — _feasible (:1099)
  S3.P.5.3 Candidate cost — _candidateCost (:1147)
  S3.P.5.4 Word finalization — _finalizeReadyWords (:1066)
  S3.P.5.5 Final verification and residual buckets — (:1219-1258)
S3.P.6 Final induced strokes (inline path) — buildFinalInducedStrokes (:1261)
S3.P.7 Extra alternate strokes (inline path) — buildExtraInducedStrokes (:1288)
S3.P.8 Report serialization (report build) — build_phase_p_realization.main (:74-112)
```

Theory 1 cannot tell apart the inflected forms of one paradigm (dors/dort, finis/finit);
their `lemmeGramCat` is the same, so Lemma-Homophone Marking (S4) does not handle them
either. Same-Lemma Disambiguation (S3) takes theory 1 and the hand-made elicitation answers
and produces the final induced strokes: each Word's base strokes plus, when the Word needs
grammatical **markers**, one extra coda-bank **marker stroke**. Marker Elicitation (Phase E)
finds every **homophone group** (same `lemmeGramCat`, same canonical Strokes) and turns the
person's per-opposition answers into, for each spelling, a list of alternate **press-sets**.
Marker Grouping (Phase G) packs the 13 live markers into the smallest number K of
**keypress groups** that still separates every spelling (K=7 today). Marker Stroke
Realization (Phase P) gives each keypress group one physical coda-bank key, greedily and
under collision checks, and appends one marker stroke per marked Word.
...
(:545-546). Nothing is asked: the run regenerates questionnaire items and applies the stored
elicitation answers.

#### Homophone group building — buildLemmaHomophoneGroups (S3.E.1)   src/elicitation.py:61
Called by: Marker Elicitation (Phase E) entry (:548); also the precedence-spec check (:148)
and the pers_3 rewrite (:169).
...
with more than one Word. Different-`lemmeGramCat` homophones are left to Lemma-Homophone
Marking (S4).
Result: homophone groups, `dict[(canonical Strokes, LemmeGramCat), list[Word]]`, 47,830, in
theory-1 order.
Notes: the key type is named `LemmaHomophoneGroupKey` (:24) although it is a same-lemma group.

#### Reading enumeration — wordFeatureCombinations / featureCombinationsByOrtho (S3.E.2)   src/elicitation.py:30, :86
Called by: every step that walks a group (Scale report (S3.E.3), Questionnaire item selection
(S3.E.4), Press-set resolution (S3.E.6) and Per-reading press resolution (S3.E.6.1)).
Transformation: a Word with verb tags yields one **reading** per tag, split into markers by
`Word.splitInfoVerb` (word.py:102): `ind:pre:1p` → {indicatif, présent, pers_1, nbr_p}. Only
participle readings also get the Word's gender and number and a `VER` marker (:47-51).
Subjonctif imparfait readings are dropped (:56, scope decision 2026-09-19). A Word without
tags yields one reading {gender, number}, possibly empty. `featureCombinationsByOrtho`
merges all Words sharing an `ortho` into one reading list.
Result: `dict[WordOrtho, list[Reading]]` per group; a reading is a `frozenset[str]` of markers.
Notes: from here on, Marker Elicitation (Phase E) reasons per spelling; Marker Stroke
Realization (Phase P) must map a spelling back to one Word (**spelling twins**, item B1).
...
:107) and prints: distinct **oppositions**, **tie oppositions** (two spellings with the same
reading — no marker can separate them), maximum reading size, co-occurring marker pairs, and
two Welsh-Powell greedy colorings (`_greedyColorCount` :126) as a rough K estimate.
Result: console only.
...
spellings — spellings with only this reading in the group — then summed corpus frequency);
the best pair is kept; items are ranked by frequency and given positional ids `q0…`.
...
Transformation: indexes answers by `frozenset({readingA, readingB})`; disagreeing duplicates
are reported and dropped, agreeing ones collapsed.
...
Called by: Marker Elicitation (Phase E) entry (:586), and again inside Press-set conflict validation (S3.E.7).
Input state: homophone groups + `AnswerByOpposition`.
Transformation: calls Per-reading press resolution (S3.E.6.1), then collapses per-reading
presses to one list per spelling, deduplicated and sorted by (size, sorted markers). Index 0,
the smallest, is the **primary alternate**. A **self-homograph** spelling ("calmez":
impératif or indicatif 2p) keeps one **alternate** per distinct reading press instead of
their union (the calmez fix). A spelling whose primary alternate is `∅` is the group's
**canonical member**.
Result: resolved press-sets, 47,828 groups; 9,692 spellings have >1 alternate; 4,858 groups
have no `∅` member, so every spelling in them is marked.
...
`readings`); precedence-spec check (:161).
Transformation: skips groups with <2 spellings (2 groups). For each cross-spelling pair of
different readings, looks up that opposition's answer and unions each side's checked markers
into that (spelling, reading)'s press. Tie oppositions are skipped (:356-357). A missing
answer (an **unresolved opposition**) drops the whole group (:360-363).
Result: `dict[group, dict[(ortho, Reading), frozenset[str]]]` + unresolved oppositions (0 today).
Notes: a reading's press is the union over every partner spelling in its group. Dropped
...
Transformation: recomputes Press-set resolution (S3.E.6) and flags any press-set held (as any
alternate) by two different spellings of one group: a **press-set conflict** (`GroupConflict`
:414). Two alternates of one spelling may be equal.
Result: 0 conflicts; conflicting groups would be dropped (:611-614).
Notes: Per-reading press resolution (S3.E.6.1) runs three times per invocation (:586, inside :587, :616) with identical results.
...
Result: `dict[group, dict[ortho, float]]`, used only by Usage weights (S3.G.4).
...
(ortho → list of sorted marker lists), `frequencies`, and `readings` (parallel to
`pressSets`: the readings that resolved to each alternate). Stable sort by `lemmeGramCat`.
...
Transformation: reads `questionnaire.json` at import (:12), injects items and French marker
labels (`LABELS` :15) into an HTML/JS page with two spellings and their marker checkboxes per
...
Input state: theory 1 + elicitation answers; recomputes Homophone group building (S3.E.1),
Answer indexing (S3.E.5) and Per-reading press resolution (S3.E.6.1) itself.
Transformation: `parsePrecedenceOrder` (:43) reads the **precedence spec**
`conjugation_disambiguation_order.txt` up to `---`, but uses it only for the marker
vocabulary (:137, "unknown atoms" report): **precedence is never enforced or checked.**
`classifyCombination` (:63) assigns each reading a family; `checkPressByOrthoCombination`
(:90) applies three rules: (1) a participle reading with gender/number ⊆ {m, s} must have an
empty press; (2) a non-empty impératif or infinitif press must be exactly that marker; (3) a
non-empty subjonctif press is `subjonctif` plus at most one `pers_*`/`nbr_*`.
...
Transformation: removed `pers_3` from every press of `elicitation_answers_pers1default.json`,
gave empty opposite sides their own `pers_1`/`pers_2`, auto-repaired press-set conflicts and
**overwrote** `elicitation_answers.json` (:177-178).
Notes: history only; a rerun would undo later hand fixes (4e73533, 688c74d, 3b22e0f).

### Marker Grouping (Phase G)

Entry: `python -m util.build_phase_g_assignment`, `main` at :49. A marker is **live** if
some resolved press-set contains it (13 today). The other 7 questionnaire markers (`VER`,
`indicatif`, `m`, `nbr_s`, `participe`, `présent`, `s`) are **unpressable**: nobody checked
them. Pressing a keypress group asserts every marker it carries. A **keypress conflict**
(`KeypressConflict` phaseg.py:150) is two spellings of one group inducing the same set of
keypress groups.
...
Transformation: sorts live markers alphabetically (:511); reduces groups to shapes; finds
the minimum K under **hard grouping rules** only; applies the **soft preference tiers** at
that K; then the alphabetical tie-break.
Result: `(numKeys=7, colorOf: marker → keypress id, achieved=[1, 1, 0])`.

- **Group shape dedup — groupSignatures (S3.G.2.1)** :40 — each group becomes its **group
  shape** (a frozenset of per-spelling alternate sets; the code type is `GroupSignature`),
  deduplicated and canonically sorted: 294 shapes from 47,828 groups.
- **Minimum-K scan — minKeypressesSat → _feasibleAssignment (S3.G.2.2)** :417, :183 — for
  K = 1…20, builds the distinctness model plus hard constraints; INFEASIBLE → next K; the
  first FEASIBLE K is the proven minimum; UNKNOWN (30 s limit) raises (:226-229). K=7.
- **Distinctness model — _buildDistinctnessModel (S3.G.2.3)** :51 — Booleans `x[m,k]`, one
  keypress per marker; `t[k]` = "this press-set touches keypress k"; every pair of press-sets
  of **different** spellings must differ on some `k` (exact XOR linearization, :96-103);
  alternates of one spelling are exempt. Mirrors Ground-truth verification (S3.G.3) exactly.
- **Hard constraints (S3.G.2.4)** :120, :108 — `aloneKeys` → (m, every other live marker)
  pairs; `mustDifferGroups` → all pairs inside; each pair gets `x[m1,k] + x[m2,k] ≤ 1`. Can
  raise K.
- **Soft preference tiers — _bestAssignmentWithPriorities (S3.G.2.5)** :358 — lexicographic
  at fixed K: tier 0 maximizes {p, nbr_p} sharing a keypress; tier 1 {future, passé}; tier 2
  minimizes other markers sharing the keypress of {nbr_p, p} (`_sameKeyScoreExpr` :308,
  `_exclusiveGroupExtraCountExpr` :329). Each tier is locked by equality before the next
  (:405). Cannot raise K.
- **Alphabetical tie-break — _breakTiesAlphabetically (S3.G.2.6)** :153 — per marker in
  alphabetical order, minimizes its keypress index and locks it. Single-threaded, seed 0
  (`_newDeterministicSolver` :139). Unique lexicographic minimum, provided every solve is
  OPTIMAL (item B24).

#### Ground-truth verification — verifyKeypressAssignment (S3.G.3)   src/phaseg.py:160
Called by: Marker Grouping (Phase G) entry (:69); also tests.
Transformation: for every alternate, computes the **induced press-set** (`inducedPressSet`
:135, the union of the keypress groups touched) and reports a keypress conflict when two
spellings induce the same set. Any conflict aborts the write (:70-71).
Result: `[]`.
...
Result: reported only; Marker Stroke Realization (Phase P) computes its own costs.
...
pers_2}, hard/soft provenance, 7 unpressable markers, usage weights.
...
answer fix); **K=7 from 688c74d** (per-reading alternates) to HEAD. The claim that the hard
constraints cost nothing extra was checked at K=6 only. The greedy `src/phaseg.py` path is
not live; only its loaders, `liveMarkers`, `verifyKeypressAssignment`, `inducedPressSet` and
`frequencyWeightedChordSizes` are.
...
marker stroke — the sorted union of those groups' keys — after its base strokes
(`_appendCodaExtraStroke` :892); its last base stroke is unchanged. Groups are decided
greedily, largest population first. The cost is the frequency-weighted `getStrokeCost` of
the real composed marker stroke. A **preferred key** per marker (`PREFERRED_KEYS_BY_MARKER`
:945: impératif → 18 `-k`, pers_2 → 19 `-d`, pers_3 → 20 `-t`) wins whenever feasible.
...
an empty one (canonical member); resolves the spelling to one Word (Entry-to-Word resolution (S3.P.2.1)) and appends it
to every keypress group whose markers intersect the primary press-set.
...
Called by: Keypress group population (S3.P.2) and Extra alternate population (S3.P.3).
...
current press-sets), only the first is marked. See TODO.md § Suspected bugs, item B1
(fallback: item B20).
...
Transformation: maps each preferred marker to its group in this run (group ids change between
runs of Marker Grouping (Phase G)); markers that are not live are skipped.
...
from Candidate ranking (S3.P.5.1), or records the group in `unassignedGroups`; runs Word
finalization (S3.P.5.4); finally runs Final verification and residual buckets (S3.P.5.5).
Result: physical keypress assignment (`KeypressGroupPhysicalAssignment` :904): g0→21, g1→16,
...
- **Candidate ranking — _bestCandidate (S3.P.5.1)** :1177 — every distinct single-phoneme
  coda key-set, keeping the feasible ones with their cost; pairs (`comboSize`=2) only if none
  is feasible. Stable sort by cost (ties keep phoneme order, :1192). A feasible preferred key
  moves or is inserted first (:1195-1200); otherwise the preference is recorded as not
  honored (:1201-1202).
- **Candidate feasibility — _feasible (S3.P.5.2)** :1099 — rejects a candidate when:
  (a) it equals an already chosen key-set (:1104); (b) it is empty or redundant for some Word;
  (c) some Word's composed marker stroke is not a legal keypress (`getStrokeCost` → None,
  :1112); (d) some composed Strokes is an existing theory-1 key (raw comparison, :1114);
  (e) a **same-lemmeGramCat collision** (`_isInScopeCollision` :968: different ortho, same
  `lemmeGramCat`) among Words needing the same full group set (:1125-1133); (f) such a
  collision with an already finalized Word (:1140-1144). Cross-category and cross-lemma
  collisions never block.
- **Candidate cost — _candidateCost (S3.P.5.3)** :1147 — `round(Σ freq·cost(composed marker
...
- **Word finalization — _finalizeReadyWords (S3.P.5.4)** :1066 — a Word whose groups are all
  decided becomes a **finalized word**, indexed in `finalizedWordsByStroke`; this catches a
  multi-group union equal to another group's key-set. Iterates the `allWords` set, which
  affects only list order inside the index.
- **Final verification and residual buckets (S3.P.5.5)** :1219-1258 — for each Word in
  `allWords` (set iteration, :1227), composes the primary stroke and one per extra alternate
  (:1232-1241). `residualTheoryCollisions` = marked Words whose composed stroke is a theory-1
  key (raw, sorted by ortho, :1243). `findCollidingInducedStrokes` (:727) pairs each key with
  the **first key seen** on the same stroke; each pair goes to `residualCollisions`
  (same-lemmeGramCat, :1248), `crossCategoryClashCollisions` (same bare lemme, different
  `lemmeGramCat`, :1251) or `crossLemmaCollisions` (:1255); same-ortho pairs are dropped.
  Today: 0 theory, 0 same-lemmeGramCat, 34 to 40 **cross-category clashes** and about 1,290
  **cross-lemma collisions** (the lists vary between clean rebuilds, item B11).
...
groups gets one marker stroke with the union of their chosen keys; others keep their base
strokes. Unassigned groups add nothing, silently.
Result: final induced strokes, 167,639 Words, 79,449 with a marker stroke. Handed to
Reserved-key composition (S4.4).
...
Transformation: for each extra alternate of a self-homograph, base strokes plus one marker
stroke with that alternate's union of chosen keys (empty key-sets skipped).
Result: `dict[Word, list[Strokes]]`, appended after index 0 in theory 2. These strokes skip
Lemma-Homophone Marking (S4) (item B4) and are never collision-checked when the primary is
empty (item B21).
...
Transformation: per group in id order: markers, `affectedWords`, `chosenKeys`, `cost`, ranked
`alternates`; then the four residual buckets as ortho pairs, in the order produced by Final
verification and residual buckets (S3.P.5.5).
Artifacts: reads pickles, `starboard3h.json`, keypress groups, resolved press-sets; writes
...
The unmarked spelling of a homophone group is chosen entirely by the elicitation answers:
for each opposition the person decides which side needs nothing (first record: `il` {m,s}
checked `[]` vs `ils` {m,p} checked `[p]`). Per-reading press resolution (S3.E.6.1) and
Press-set resolution (S3.E.6) turn an empty union into the `∅` primary alternate; Keypress
group population (S3.P.2) skips it. No code computes it; 4,858 groups have none. The
precedence spec only states the intended default ("m" / "m s" first), and the precedence-spec
check verifies it for participles only. `FEATURE_PRIORITY` (src/greedyoptimizer.py:16) is not
used on this path; `GRAMCAT_PRIORITY` (:62) is live only in Lemma-Homophone Marking (S4).

---
## Lemma-Homophone Marking (S4)

Marker Stroke Realization (Phase P) separates only words with the same `lemmeGramCat`.
Collisions across lemmas ("ver/vert/verre") and across categories of one lemma
("appel"/"appelle") remain. Lemma-Homophone Marking (S4) takes the final induced strokes,
finds every group of Words that share one canonical stroke **and** span ≥2 `lemmeGramCat`s
and ≥2 spellings (a **lemma-homophone cluster**), merges homographs and 1990-reform
**doublets**, ranks the cluster from most canonical to most marked with the **marking rule
stack**, and gives each rank a **mark code**: `()`, `*`, `#`, `*#`, then escalated `*#`
codes. The first symbol is pressed with the word's **last phoneme stroke** (a **merged
mark**); further symbols become **bare mark strokes**. The result is theory 2, in memory,
plus the human view `theory2.tsv`.

Scale: 4,450 lemma-homophone clusters. Sizes (Words): 2: 2,947; 3: 989; 4: 341; 5: 104; 6:
40; 7: 19; 8: 7; 10: 1; 11: 2. Codes: `()` 5,530, `*` 4,886, `#` 579, `*#` 146, `(*#,*#)` 51,
`(*#)×3` 9, `(*#)×4` 3, `(*#)×5` 2 — 5,676 marked Words, 86 bare mark strokes.

### Theory 2 assembly — Dictionary.buildFinalTheory (S4.1)   dictionary.py:342
Called by: Phonetic Theory Building (S2) `__main__` (dictionary.py:532, only when both
`phase_g_keypress_assignment.json` and `resolved_press_sets.json` exist, :531) and every
Theory Export (S5) exporter through Theory 2 loading (S5.1).
Input state: theory 1 (80,725 keys / 167,639 Words), keyboard layout, the two JSON paths.
Transformation: (1) loads `markersByKeypress` (:365-369) and the resolved press-sets
(:370-371); (2) runs Marker Stroke Realization (Phase P) inline (S4.2) → final induced
strokes; (3) calls Reserved-key composition (S4.4) with Reform-doublet loading (S4.3) and
`phonemeStrokeCounts` = each Word's base stroke count, which turns on the merge; (4) calls
Extra alternate strokes (S4.14); (5) returns `{word: [primaryComposed[word]] +
...
### Marker Stroke Realization (Phase P), inline path (S4.2)   dictionary.py:373-384
Called by: Theory 2 assembly (S4.1).
Transformation: Word lookup indexes (S3.P.1) → Keypress group population (S3.P.2) → Extra
alternate population (S3.P.3) → Preferred key resolution (S3.P.4) → Coda key search (S3.P.5)
→ Final induced strokes (S3.P.6), over the whole lexicon. Described under Same-Lemma
Disambiguation (S3).
Result: final induced strokes + physical keypress assignment (0→21, 1→16, 2→20, 3→23, 4→18,
...
Transformation: `composed = dict(finalInduced)` (:434); for each lemma-homophone cluster from
Lemma-homophone cluster detection (S4.5), gets each member's mark strokes from Physical mark
assignment (S4.6); Words with code `()` keep their strokes, the others go through Mark merge
into the last phoneme stroke (S4.13).
...
primary strokes only, not alternates.
...
Result: 4,450 lemma-homophone clusters.
Notes: a bucket with one `lemmeGramCat` and several spellings is dropped on purpose (a code
comment leaves it to Marker Stroke Realization (Phase P)): 98 such buckets survive into theory 2, all from spelling twins (item B1).
Same-lemma cross-category clashes ("appel" NOM / "appelle" VER) are in scope because their
`lemmeGramCat`s differ.
...
Transformation: `{word: starHashCodeToStrokes(code)}` over Mark code assignment (S4.7) and
Mark code realization (S4.12).
Result: `dict[Word, Strokes]` of mark strokes; `()` = no mark.
...
3. Each merged group's **mark representative** = `max(group, key=frequency)` (:332).
4. Cluster ranking (S4.8) of the representatives; Mark code sequence (S4.11) for their count;
   zip in rank order (:333-335).
5. Every Word of a merged group gets its representative's code, stored by `ortho` (:337-343).
Result: `dict[Word, tuple[str, ...]]`; homographs and doublets share a code. 18 clusters end
...
Transformation: `a is b` → 0. Else `marked = decideStarHashMark(a, b)` (Pairwise marking decision (S4.10)); `None` →
higher frequency first, then `ortho` ascending, then 0; otherwise +1 if `a` is marked, −1 if `b`.
Result: −1/0/+1.
...
### Pairwise marking decision — decideStarHashMark (S4.10)   src/ambiguitychecker.py:183
Called by: Ranking comparator (S4.9), :253.
Transformation: the marking rule stack; the first matching rule decides. "Mark X" = X is less
canonical. Frequency is the film frequency.

| Rule | Line | Condition | Outcome |
|---|---|---|---|
| R1 homograph exemption | :211 | same `ortho` | `None` (no mark) |
| R2 reform-doublet exemption | :214 | `{lemmeA, lemmeB}` in the 259 doublet pairs | `None` |
| R3 per-pair override | :217-219 | `MARKING_OVERRIDES[frozenset({orthoA, orthoB})]` exists (49 entries, :130-180, keyed by ortho) | mark the stored spelling |
| R4 frequency-ratio rule | :221-225 | `hi/lo ≥ RATIO_EXEMPTION_THRESHOLD` (10.0, :91), `inf` if `lo == 0` | mark the rarer; tie → first argument |
| R5 same-category rule | :224-225 | equal `GramCat` enums (`ADJ` ≠ `ADJ:pos`) | mark the rarer; tie → first argument |
| R6 category-priority rule | :227-230 | both categories in `GRAMCAT_PRIORITY` (src/greedyoptimizer.py:62) | mark the lower priority |
| R7 frequency fallback | :232 | a category is missing from the table | mark the rarer; tie → first argument |
...
Rule usage among representative pairs: R4 4,284 · R5 854 · R6 838 · R7 93 · R3 45 · R2 0 · R1 0.
Notes: the code calls R4 a "ratio exemption"; it exempts the pair from the category rule,
not from marking. Any zero-frequency word loses to any non-zero word. The code docstring
numbers R1-R6 the same way; comments at :392/:431 and a RESUME note use another numbering.

### Mark code sequence — assignStarHashCombos (S4.11)   src/ambiguitychecker.py:272
Called by: Mark code assignment (S4.7), :334.
Transformation: base budget `[(), ('*',), ('#',), ('*#',)]`, then `('*#',) * n` for n = 2, 3,
… (:284-288), truncated to the rank count. Codes beyond the base budget are **escalated codes**.
...
possible third mark and never used.
Result: reserved-only strokes; `()` for the canonical member.

### Mark merge into the last phoneme stroke (S4.13)   src/ambiguitychecker.py:437-444
Called by: Reserved-key composition (S4.4).
Input state: a marked Word's final induced strokes `s` (base strokes, then maybe its marker
stroke) and its mark strokes `extra`.
...
s[last+1:] + extra[1:]`: the first mark symbol joins the last phoneme stroke (not the marker
stroke); remaining symbols follow as bare mark strokes. Without counts (tests only) it
appends `s + extra`.
Result: `pâts` `((4,12),(17,))` + `*` → `((4,12,10),(17,))` = `p*a/-s`; `aulx` `((12,14),)` +
`(*#)×5` → `*ae#/*#/*#/*#/*#`.

### Extra alternate strokes (S4.14)   src/ambiguitychecker.py:1288
Called by: Theory 2 assembly (S4.1), dictionary.py:389. Described as Extra alternate strokes (S3.P.7).
Notes: known scope gap with a measured cost: an unmarked alternate can take the only stroke of
an unrelated word. In the Plover dictionary `subits` (`s@i/svi/-s`) loses to `subis`'s
alternate, `pais` (`pie/-k`) to `paie`, `amplis` to `emplis`; also bénits, brandys, bégaies,
baillez, baryes, repais — 9 spellings without an entry (item B4).

### Theory 2 report — Dictionary.writeFinalTheory (S4.15)   dictionary.py:392
Called by: `__main__` dictionary.py:533, right after Theory 2 assembly (S4.1).
...
then trailing strokes as key-index lists joined by `/`.
...
### Cross-category clash detector — detectCrossCategoryClash (S4.16, off-pipeline)   src/ambiguitychecker.py:59
...
`lemmeGramCat` filter of Lemma-homophone cluster detection (S4.5).

### Worked examples (2026-09-22 data)

1. **R6 category rule.** Cluster `((12,), (4,13,14,23))`: `appel` NOM 80.88, `appelle` VER
   485.77. Ratio 6.0 < 10, categories differ, NOM 30 > VER 20 → `appelle` is marked although
   6× more frequent. Plover: `a/piel` → appel, `a/p*iel` → appelle; its alternates
   `a/piel/-k`, `a/piel/-l` carry no mark.
2. **Homograph merge, R4 and R7.** Cluster `((12,),)`: `à` PRE 12,190.4; `a` AUX 6,350.91 /
   VER 5,498.34 / NOM 81.36 (representative AUX); `ah` ONO 576.53; `ha` ONO 21.54 / NOM 2.94.
   à vs a: ratio 1.9, PRE not in the table → R7 → `a` marked; ah vs a: ratio 11.0 → R4; `ha`
   loses to all by R4. Codes: à `()`, a `*`, ah `#`, ha `*#` → Plover `a`, `*a`, `a#`, `*a#`.
3. **Escalation.** Cluster `((12,14),)` (11 Words, 8 representatives): au ART:def `()`; oh
   ONO `*`; aux ART:def `#` (vs oh: ratio 1.07 → R7); eau NOM `*#`; haut `*# *#`; ho
   `(*#)×3`; ô `(*#)×4`; aulx NOM 0.0 `(*#)×5`. Plover `ae`, `*ae`, `ae#`, `*ae#`,
   `*ae#/*#`, …, `*ae#/*#/*#/*#/*#`.
4. **Doublet.** Cluster `((3,5,13),(7,9,11,13))`: `bizut` 2.05 and `bizuths` 0.0 (lemmas
   bizut/bizuth, a reform pair) are merged and both get `()`; Plover keeps `bizut`.
5. **Tie decided by input order.** `pas` NOM 0.0 vs `pâts` NOM 0.0: R4 (lo = 0) marks the
   first argument; the insertion sort calls `compare(later, earlier)`, so the later Word in
   theory-1 order is marked (`pâts` → `p*a/-s`); reversed input marks `pas` (item B5).

---

## Theory Export (S5)

Theory Export (S5) turns theory 2 into the files people use. Every exporter that needs
theory 2 (Plover dictionary export (S5.3) and the three drill exporters) recomputes it in its
own process; none reads `theory2.tsv`. The Plover key table export (S5.4) and the trainer
keyboard legend (S5.6) read only `starboard3h.json`, and the legend also reads the
realization report.

### Theory 2 loading — loadFirstAndFinalTheory / loadFinalTheory (S5.1)   util/_theoryio.py:68, :48
Called by: Plover dictionary export (S5.3) via `loadFinalTheory`; Trainer word drill (S5.7),
Trainer sentences (S5.8) and Trainer definitions (S5.9) via `loadFirstAndFinalTheory`. The
report build of Marker Stroke Realization (Phase P) uses only `loadFirstTheory` (:42).
...
Called by: Plover dictionary export (S5.3, :45), Trainer word drill (S5.7, :231), Trainer
sentences (S5.8, :164), Trainer definitions (S5.9, :65).
Transformation: per stroke, `keys = sorted(set(stroke))` (the canonical form), then:
1. **bare mark stroke** (only keys 10/15) → `*`, `#` or `*#` (:43-44);
2. **merged mark stroke** (reserved + phoneme keys) → `_renderMarkedStroke` (:26), Plover's
   `plover_stroke` hyphen rule: **implicit-hyphen keys** are nucleus 11-14 plus `STAR_KEY` 10;
   `firstRightKey` = 15; a `-` precedes the first key ≥15 only without an implicit-hyphen key
   (`*iel`, `ie#l`, `swa#`, `pvR-#`);
...
`*`/`#`; 58 end in a bare `*#` stroke. 29 spellings have no entry: 20 reform-doublet or
near-doublet losers (`bizuths`, `dégottés`, `toquade`, `cuissot`, …) and 9 alternate-shadowed spellings (item B4).
...
### Plover system plugin (S5.5)   plover_stenalgo/plover_stenalgo/system.py
...
Notes: bare `*` as undo is safe because the first mark symbol is always merged and escalated
bare strokes are always `*#`. The user adds `plover_stenalgo_dictionary.json` by hand.
...
phoneme layers (`_phonemeLayers` :81); the **conjugation-marker legend**
(`_conjugationMarkers` :117) from the realization report (:128): per keypress group,
`chosenKeys`, key names and French marker labels. Does not call `buildFinalTheory`.
Result: `steno-trainer/public/data/keyboard-layout.json`.
Notes: inline and report keys agree today (0→21 … 6→19); nothing detects a mismatch (item
B18). A null `chosenKeys` would crash (item B19).
...
Transformation: `buildReadingsByWord` (:175) maps each press-set entry's `readings` to a Word
through `_resolveEntryWord`; `chordsWithReadings` (:194) pairs each theory-2 stroke with its
readings (on a count mismatch, every stroke gets all readings and the Word counts as
"misaligned"). **Drill items** are keyed by (ortho, steno); a second Word with the same key
merges label and context (:233-245). Each carries context words (`formatContext` :133), a
French label (`formatReadingsLabel` :112), dotted phonology, steno, sorted key strokes and film
frequency. Sorted by (−frequency, ortho, steno), truncated to 10,000.
...
Called by: `python -m util.export_practice_sentences`, after Trainer word drill (S5.7).
